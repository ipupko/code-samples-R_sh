...
input_dir <- args[1] #Should be output directory {$output_dir}{$input_file}.sexcheck
output_dir <- args[2] #I will change it to report directory
file_F.stat_y_count <- args[3] #This is done 
report_dir<-args[4]

setwd(input_dir)  # Purge
library(tidyverse) # Purge
library(data.table)
...
pdf(paste0(report_dir, "Sex_check.pdf"))
# Whole sample
...
output_file <- paste0(report_dir, "report_sex.mismatch_aneuploidies.txt")
...
sex_mismatch_overall<-rbind(sex_mismatch_females,sex_mismatch_males)

sex_mismatch_overall_ids<-sex_mismatch_overall[,c(1,2)]
#### TURNER SYNDROM
...
write.table(id_list_aneuploidies,paste0(report_dir,"id_list_aneuploidies.txt"), quote=F,row.names=F,sep="\t",col.names=F)
write.table(sex_mismatch_overall_ids,paste0(report_dir,"id_list_sex_mismatch.txt"), quote=F,row.names=F,sep="\t",col.names=F)
